...
class UserModelCase(unittest.TestCase):
    ...
    def setUp(self):
        app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite://'  # хак - база для тестов создается в памяти
        db.create_all()  # создаются все таблицы как в изначальной бд
    ...
    def test_avatar(self):
        ...
        self.assertEqual(u.avatar(128), ('https://www.gravatar.com/avatar/'
                                         'f3fc30174d7fd74ab6ca3c36d198fcb9'
                                         '?d=identicon&s=128'))

    def test_follow(self):
        u1 = User(username='Susan', email='Susan@example.org')
        u2 = User(username='John', email='John@example.org')
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
